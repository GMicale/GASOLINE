...
public class GASOLINE
{
	...
	{
		//Input parameters
		String pathNetworks=null;
		String homologyFile=null;
		String pathOutput=null;
		int sigma=3;
		double overlapThresh=0.5;
		int iterIterative=20;
		int minAlignSize=4;
		double densityThresh=0.8;
		String help = "Usage: java -jar Gasoline.jar -i <networkFolder>" +
		" [-sim <orthoSimFile> -sig <sigma> -ds <densityThresh> -ov <overlapThresh> -it <iterIterative> -ms <minAlignSize> -o <outputFolder>]";
		
		//Assign input parameter values (unrecognizable commands are ignored)
		int i=0;
		for (i=0; i<args.length; i++) 
		{
			if(args[i].equals("-i"))
				...
				sigma=Integer.parseInt(args[++i]);
			else if(args[i].equals("-it"))
				iterIterative=Integer.parseInt(args[++i]);
			else if(args[i].equals("-ov"))
				overlapThresh=Double.parseDouble(args[++i]);
			else if(args[i].equals("-ds"))
				densityThresh=Double.parseDouble(args[++i]);
			else if(args[i].equals("-ms"))
				minAlignSize= Integer.parseInt(args[++i]);
			else 
			{
		...
		//Error in case network files are missing
		if(pathNetworks==null) 
		{
			System.out.println("Error! No folder for input networks has been specified!");
			System.out.println(help);
			System.exit(1);
		}
		System.out.println("\nRunning GASOLINE with the following parameters:");
		System.out.println("INPUT PATH: "+pathNetworks);
		if(homologyFile==null)
			System.out.println("HOMOLOGY FILE: no homology file specified. Use protein names to evaluate similarities.");
		else
			System.out.println("HOMOLOGY FILE: "+homologyFile);
		if(pathOutput==null)
			System.out.println("OUTPUT PATH: no output path specified. Do not save results to disk.");
		else
			System.out.println("OUTPUT PATH: "+pathOutput);
		System.out.println("SIGMA: "+sigma);
		System.out.println("DENSITY THRESHOLD: "+densityThresh);
		System.out.println("OVERLAP THRESHOLD: "+overlapThresh);
		System.out.println("NUMBER OF ITERATION OF ITERATIVE PHASE: "+iterIterative);
		System.out.println("MINIMUM SIZE OF ALIGNMENTS: "+minAlignSize);
		System.out.println();
		...
		//Read input networks
		System.out.println("Reading input networks...");
		Graph[] g=fileMan.readGraphs(pathNetworks);
		
		//Read homology scores
		HashMap<String,Double> mapHomology=null;
		if(homologyFile!=null)
		{
			System.out.println("Reading orthology file...");
		...
		System.out.println("Filtering seed nodes...");
		HashSet<String>[] startingNodes=filterStartingNodes(g, sigma);
		int numExec=Integer.MAX_VALUE;
		for(i=0;i<startingNodes.length;i++)
		{
			if(startingNodes[i].size()<numExec)
				numExec=startingNodes[i].size();
		}
		
		//Start search
		System.out.println("Searching complexes...");
		for(indIter=1;indIter<=numExec;indIter++)
		{
			...
			if(!different)
			{
				...
				for(i=0;i<iterIterative;i++)
				{
					if(finalAlign[0].size()>=3)
						removeNode(g,finalAlign,densityThresh);
					extendSeeds(g,finalAlign,mapHomology,iterGibbs,densityThresh);
				...
				if(mapping[0].size()>=minAlignSize)
				{
			...
			if(indIter%500==0)
				System.out.println("Completed "+indIter+"/"+numExec+" iterations..."); 
		}
		
		//PHASE 4) Postprocessing-filter overlapping complexes
		System.out.println("Filtering overlapping alignments...");
		checkOverlap(rankAlign, overlapThresh);
        
		//Print final set of local alignments
		fileMan.writeAlignments(g, rankAlign, pathOutput);
		if(pathOutput!=null)
			System.out.println("Alignments correctly saved in "+pathOutput);
